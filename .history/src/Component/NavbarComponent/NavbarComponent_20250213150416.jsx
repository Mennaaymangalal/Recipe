...
export default function NavbarComponent() {
  ...
  return (
    ...
    <div className=" text-white h-screen  p-4">
      <img src={logo} alt="" />
      <div className="flex flex-col h-full gap-6 w-4/5 align-middle pt-10">        
        {menuItems.map((item,index)=>{
          return <div key={index} className="">
            <NavLink to={item == "Meals" ? "/" : "/" + item.toLocaleLowerCase()}
             className={({ isActive }) =>
              `px-4 py-2 rounded-md w-full block text-left textx  hover:scale-105 transition-all border border-gray-200 ${
                isActive
